...
`Program.cs` is the composition root: dispatches on `args[0]` to Run (default, `SyncWorker` hosted service) / `login` / `test` / `healthcheck`, and wires DI. `healthcheck` short-circuits before the host is built (see ADR 0006) and just checks `Health/HeartbeatCheck.cs` against the deadline `Health/HeartbeatWriter.cs` writes each Sync Cycle. Flow of a **Sync Cycle** (`Sync/SyncWorker.cs`):
